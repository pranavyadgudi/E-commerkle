...
app.post("/cart/add", async (req, res) => {
  const body = req.body;
  const productsArray = body.products; // List of product IDs (not full product objects)
  let totalPrice = 0;
  ...
  try {
    ...
    if (user.cart) {
      ...
      // Add new products only if not already in the cart
      for (const productId of productsArray) {
        if (!existingProductIds.includes(productId.toString())) {
          cart.products.push(productId); // Push ObjectId here
      ...
      // Create new cart if the user doesn't have one
      cart = new Cart({
        products: productsArray, // This should be an array of ObjectIds
        total: totalPrice,
      });
